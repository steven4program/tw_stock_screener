...
- 每檔的 `director_holding_pct` / `director_data_month` 取自 `director_holdings_monthly` 中**最新可用月份**。
- 若本次董監抓取失敗、或當月尚未公布 → **沿用最新可用月份**，`job_runs.status = partial_success`，並在快照 `director_data_month` 與前端清楚標示所用的資料月份。

**交易日判斷原則**

- 系統**不自行維護交易日曆**；「最新交易日」一律以資料來源回傳的最新 `data_date` 為準（見步驟 1–2）。週末／假日或尚無新資料 → `no_new_data`，不覆蓋既有快照。
...
> `partial_success` 定義：部分資料來源成功（例如股價、法人成功但董監抓取失敗），仍寫出當日快照但標記為 partial。董監失敗時**沿用最新可用董監月份**，並於快照 `director_data_month` 與前端清楚標示資料月份。
...
UI 控制限制在上述範圍以避免誤調（超出範圍不可再增減）；其餘為固定參數，MVP 不開放調整。
...
**連買天數計算**：以 `institutional_daily.net_lots > 0` 的連續天數計；當日無法人資料（未交易）視為中斷、歸零（保守）。`inst_buy_streak` 若超過已回補可確認範圍，顯示為「≥已確認天數」；MVP 因 `N` 上限為 10，回補 30 日已足以支援篩選。
...
> **視覺設計來源**：畫面的視覺與版面由 **Claude Design** 產出（使用者提供設計稿），前端依設計稿實作。本節定義的是**資訊架構與功能需求**，不取代視覺設計；撰寫前端實作計畫時，會以 Claude Design 的設計稿為準對齊版面、元件與樣式。

**頂部狀態列**

- 大字顯示更新狀態：`✅ 今日已更新 ・ 資料日期 2026-06-09` 或 `⚠️ 今日尚未更新（沿用 2026-06-06 資料）`，資料來源為 `job_runs` 與快照 `data_date`。
- 部分成功（`partial_success`）時加註提示，例如 `⚠️ 董監資料沿用 2026-05 月份`。

**參數摘要區**

- 顯示本次篩選參數：法人連買 ≥ N 天、董監持股 ≥ X%、距均線 0~10%、月線 20MA、季線 60MA、扣抵 5 個交易日、資料日期。
- `N`（1–10）、`X`（5–50）提供大顆 +/− 控制即時調整，超出範圍不可再增減；其餘參數固定顯示。
...
- 共同欄位：代號、名稱、類型標籤（A/B/A+B）、收盤價、漲跌幅%、連買天數、買超張數、董監持股%（旁標示資料月份 `director_data_month`）、成交量。
...
## 11. 實作前置：董監持股資料 POC —— 已驗證 ✅

- **董監持股確切來源端點與解析方式（已由 POC 確定）**：詳見 [`poc/director-holdings/FINDINGS.md`](../../../poc/director-holdings/FINDINGS.md)。結論為 **GO**。摘要：
  - **董監明細（批次、全市場一次抓）**：上市 `https://openapi.twse.com.tw/v1/opendata/t187ap11_L`、上櫃 `https://www.tpex.org.tw/openapi/v1/mopsfin_t187ap11_O`（皆送 `Accept: text/csv`）。
  - **已發行股數**：上市 `t187ap03_L` 欄位 `已發行普通股數或TDR原股發行股數`；上櫃 `mopsfin_t187ap03_O` 欄位 `IssueShares`（**直接取已發行股數欄位，不用資本額估算**）。
  - **彙總**：明細為「全體內部人」，須**只計董監本人（職稱含董事/監察人且不含法人代表）並依姓名去重**，再 `Σ董監股數 ÷ 已發行股數`。已與 MOPS 官方 `全體董監持股合計` **逐股完全相符**（2330=6.52%、6488=46.96%，差 0.00pp）。
  - **涵蓋率（普查）**：上市 98.81%、上櫃 99.66%（缺漏多為 TDR/外國發行人，本無董監資料）。
  - **更新頻率／殘留風險**：上市明細每月（data.gov 22811 確認）；上櫃明細 data.gov 標示「每1年」（22812），故管線需加「資料年月新鮮度守門」，並保留 **MOPS（`mopsov.twse.com.tw/mops/web/ajax_stapap1`）每月逐檔備援**（已驗證可行）。
  - `director_holdings_monthly` 寫入流程：每月觸發／每日檢查當月明細是否已公布（出表約次月中旬），寫入時記錄 `director_data_month`。

- **POC 為第一個里程碑、且已完成**；其餘部分（資料管線、訊號計算、篩選引擎、API、前端）於 POC 通過後展開。
...
本工具僅為**個人選股資訊整理**，不構成任何投資建議或買賣推薦。

- 資料來自第三方公開來源（FinMind、公開資訊觀測站），可能有**延遲、缺漏或錯誤**；董監持股為每月更新，且可能沿用較早月份（見 §4）。
- 均線、扣抵等計算僅供參考，不保證正確或即時。
- 使用者應自行查證並獨立判斷，任何投資決策與盈虧由使用者自行負責。
